...
void Map_World::Map_Construcion(int i, int &Present_Money, int &Toll, int k)
{ //변수 i는 플레이어 자기 자신을 의미한다.
	Toll = 0;
	if (GetLand_Owner() == 4)
	{
		int Buy_Land = 0;
...
void Map_World::Construction_Str(int i, int &Present_Money)
{
	...
	while (1)
	{
		...
		if (Buy_Building == 1)
		{
			if (GetVilla_Owner() == i)
			{
				cout << "이미 이 땅의 빌라를 가지고 있습니다." << endl
					 << endl;
			}
		...
		else if (Buy_Building == 2)
		{
			if (GetBuilding_Owner() == i)
			{
				cout << "이미 이 땅의 빌딩을 가지고 있습니다." << endl
					 << endl;
			}
		...
		else if (Buy_Building == 3)
		{
			if (GetHotel_Owner() == i)
			{
				cout << "이미 이 땅의 호텔을 가지고 있습니다." << endl
					 << endl;
			}
